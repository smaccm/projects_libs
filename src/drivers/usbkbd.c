...
kbd_update_ind(usb_kbd_t kbd)
{
    ...
    *kbd->req = __set_report(HID_OUTPUT, 0, kbd->ifno, 1);
    return usbdev_schedule_xact(kbd->udev, 0, kbd->udev->max_pkt, 0, kbd->xact, 2,
                                NULL, NULL);
}
...
kbd_update_repeat_rate(usb_kbd_t kbd)
{
    ...
    *kbd->req = __set_idle_req(kbd->repeat_rate, kbd->ifno);
    return usbdev_schedule_xact(kbd->udev, 0, kbd->udev->max_pkt, 0,
...
kbd_connect(usb_dev_t udev)
{
    ...
    *req = __set_idle_req(0, kbd->ifno);
    ...
    usbdev_schedule_xact(udev, kbd->int_ep, kbd->int_max_pkt,
                         kbd->int_rate_ms, kbd->int_xact, 1,
                         &kbd_irq_handler, kbd);
#else
