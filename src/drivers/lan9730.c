...
#define EP_IN_SIZE    512
#define EP_OUT_SIZE   512
#define EP_INT_SIZE    16
...
static inline struct usbreq
__clear_remote_wakeup(uint16_t ep) {
    return __clear_ep_feature_req(ep, FEAT_REMOTE_WAKEUP);
}

static inline struct usbreq
__clear_epstall(uint16_t ep) {
...
static int write_register(struct usb_eth* eth, int addr, uint32_t v)
{
    ...
    r = dma_vaddr(eth->reg_write_xact[0].buf);
    d = dma_vaddr(eth->reg_write_xact[1].buf);
...
static int read_register(struct usb_eth* eth, int addr, uint32_t *v)
{
    ...
    r = dma_vaddr(eth->reg_read_xact[0].buf);
    d = dma_vaddr(eth->reg_read_xact[1].buf);
...
static int
do_lan9730_input(struct netif *netif, struct xact *xact)
{
    ...
    uint32_t sts;
    int len;
    hdr = (uint32_t*)dma_vaddr(xact->buf);
    payload = (char*)&hdr[1];
    sts = *hdr;
...
static int
eth_process_status(struct netif* netif, uint32_t status)
{
    struct usb_eth* eth;
...
static int
eth_config_cb(void* token, int cfg, int iface,
              struct anon_desc* d)
{
    struct usb_eth* eth = (struct usb_eth*)token;
    assert(eth);
    if (d) {
        ...
        case ENDPOINT: {
            ...
            eth->int_ep = e->bEndpointAddress & 0xf;
            eth->int_max_pkt = e->wMaxPacketSize;
            eth->int_rate_ms = e->bInterval * 2;
            eth->ifno = iface;
            eth->cfgno = cfg;
            break;
        }
...
static err_t tx_packet(struct usb_eth* eth, struct pbuf* p)
{
    ...
    xact[1].type = PID_OUT;
    xact[1].len = 0;
    err = usb_alloc_xact(eth->udev, xact, 2);
    if (err) {
        return ENOMEM;
    }
    hdr = (uint32_t*)dma_vaddr(xact[0].buf);
    payload = (char*)&hdr[2];
    /* Construct the packet */
...
int
lan9730_input(struct netif *netif)
{
    struct usb_eth* eth;
    struct xact xact;
    int err;
    ...
    xact.type = PID_IN;
    xact.len = EP_IN_SIZE;
    err = usb_alloc_xact(eth->udev, &xact, 1);
    if (err) {
        return -1;
    }
    /* Read in a frame */
...
err_t
lan9730_linkoutput(struct netif *netif, struct pbuf *p)
{
...
err_t
lan9730_init(struct netif* netif)
{
...
struct netif* lan9730_driver_bind(usb_dev_t udev) {
    ...
    eth->udev = udev;
    ETH_DBG(eth, "Parsing config\n");
    eth->int_ep = -1;
    err = usbdev_parse_config(eth->udev, &eth_config_cb, eth);
    if (err || eth->int_ep == -1) {
        assert(0);
        return NULL;
    }
    ...
    *req = __set_interface_req(eth->ifno);
    ...
    err = usb_alloc_xact(eth->udev, eth->reg_read_xact, 2);
    if (err) {
        return NULL;
    }
    err = usb_alloc_xact(eth->udev, eth->reg_write_xact, 2);
    if (err) {
        return NULL;
    }
    ...
    err = usb_alloc_xact(udev, &eth->int_xact, 1);
    assert(!err);
    eth->intbm = dma_vaddr(eth->int_xact.buf);
...
int
lan9730_poll_status(struct netif *netif)
{
    struct usb_eth* eth;
    struct xact xact[1];
    uint32_t *status;
    ...
    err = usb_alloc_xact(eth->udev, xact, 1);
    assert(!err);
    status = (uint32_t*)dma_vaddr(xact[0].buf);
